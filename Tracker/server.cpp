...
std::list<std::string> g_deny_from_clients;
...
void read_db_torrents_sql()
{
	try
	{
		...
		g_deny_from_clients.clear();
		while (Csql_row row = result.fetch_row())
			g_deny_from_clients.push_back(row[0].s());
	}
	catch (bad_query&)
	{
	}
	try
	{
		...
		{
			Csql_result result = Csql_query(m_database, "select info_hash, @fid, down_multi, up_multi from @files where flags & 2").execute();
			while (Csql_row row = result.fetch_row())
			{
				if (t_torrent* i = find_ptr(m_torrents, to_array<char, 20>(row[0])))
				{
					i->down_multi = row[2].f();
					i->up_multi = row[3].f();
				}
		...
		if (m_config.m_auto_register && !m_torrents.empty())
			return;
		Csql_result result = Csql_query(m_database, "select info_hash, @completed, @fid, ctime, down_multi, up_multi from @files where @fid >= ?")(m_fid_end).execute();
		...
		while (Csql_row row = result.fetch_row())
		{
			...
			file.fid = row[2].i();
			file.ctime = row[3].i();
			file.down_multi = row[4].f();
			file.up_multi = row[5].f();
		}
	}
	catch (bad_query&)
	{
...
void read_db_users()
{
	...
	try
	{
		Csql_query q(m_database, "select @uid, can_announce");
...
void write_db_users()
{
	...
	if (!m_torrents_users_updates_buffer.empty())
	{
		...
		async_query("insert into " + db_name("files_users") + " (active, announced, completed, downloaded, `left`, uploaded, completed_time, down_rate, up_rate, peer_id, ipa, port, mtime, fid, uid) values "
			...
			+ "  ipa = if(values(ipa) = -1, ipa, values(ipa)),"  		 
			+ "  port = if(values(port) = -1, port, values(port)),"  		
...
void clean_up(t_torrent& t, time_t time)
{
	for (auto i = t.peers.begin(); i != t.peers.end(); )
	{
		if (i->second.mtime < time)
		{
			...
			if (0) // i->second.uid)
				m_torrents_users_updates_buffer += Csql_query(m_database, "(0,0,0,0,18446744073709551615,0,-1,-1,-1,?,?),")(t.fid)(i->second.uid).read();
			t.peers.erase(i++);
			t.dirty = true;
		}
		else
...
std::string srv_insert_peer(const Ctracker_input& v, bool udp, t_user* user)
{
	...
	BOOST_FOREACH(auto& i, g_deny_from_clients)
	{
	...
	if (!file.ctime)
		file.ctime = srv_time();
	if (v.m_left && user && user->wait_time && file.down_multi && file.ctime + user->wait_time > srv_time())
	...
	if (m_use_sql && user && file.fid)
	{
			...
			&& v.m_uploaded >= i->uploaded)
		{
			...
			if (time_t t = srv_time() - i->mtime)
			{
				down_rate = downloaded / t;
				up_rate = uploaded / t;
			}
		}
		m_torrents_users_updates_buffer += Csql_query(m_database, "(?,1,?,?,?,?,?,?,?,?,?,?,?,?,?),")
			...
			(v.m_left)
			(uploaded)
			(v.m_event == Ctracker_input::e_completed ? srv_time() : -1)
			(down_rate)
			(up_rate)
			(v.m_peer_id)			
      (ntohl(v.m_ipa))
      (ntohs(v.m_port))
		...
		if (downloaded || uploaded)
		{
			m_users_updates_buffer += Csql_query(m_database, "(?,?,?),")
				.p(downloaded * file.down_multi / 100)
				.p(uploaded * file.up_multi / 100)
				.p(user->uid)
				.read();
		}
	}
